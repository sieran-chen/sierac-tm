...
- Collector 已配置 GitLab（`GITLAB_URL`、`GITLAB_TOKEN`、`GITLAB_GROUP_ID`）或 GitHub（`GITHUB_TOKEN`，可选 `GITHUB_ORG`）
- 管理端可访问 Collector API（`VITE_API_KEY` / `INTERNAL_API_KEY` 一致）
- 若用 GitLab 自动创建，至少一个成员在 GitLab 中存在且邮箱与 Cursor 一致

## 验证步骤

1. **立项（自动创建仓库）**
   - 登录管理端 → 项目 → 新建项目
   - 选择「自动创建（GitLab）」或「自动创建（GitHub）」，填写项目名称、工作目录规则、仓库 slug、创建人邮箱
   - 保存后应看到「创建成功」及 HTTP/SSH clone 地址，可一键复制

2. **仓库与 Hook**
   - 在 GitLab 或 GitHub 中确认新仓库已创建，且含 `.cursor/hook/` 目录（cursor_hook.py、hook_config.json、hooks.json）
...
7. **重试注入（可选）**
   ...
   - 再次在 GitLab 或 GitHub 查看默认分支最新提交，应包含 Hook 文件更新
...
- 立项后 GitLab 或 GitHub 仓库与 clone 地址可见
- Hook 文件存在于仓库且配置正确
- 白名单目录放行、非白名单拦截
